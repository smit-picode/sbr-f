...
// Checks for <baseKey>.view, <baseKey>.edit, <baseKey>.search, <baseKey>.view_detail in the user's permission list.
export function usePermission(baseKey: string) {
  ...
  if (isSuperAdmin) return { canView: true, canEdit: true, canSearch: true, canViewDetail: true };

...
import { EST_STATUS_OPTIONS, SECTOR_OPTIONS } from '@/constants';
import { useTranslation } from 'react-i18next';

interface EnterprisesFiltersProps {
  filters: EnterpriseFilters;
  onFilterChange: (filters: Partial<EnterpriseFilters>) => void;
  onReset: () => void;
  isDefault?: boolean;
}

export function EnterprisesFiltersBar({ filters, onFilterChange, onReset, isDefault = false }: EnterprisesFiltersProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 p-4 bg-white border border-slate-200 rounded-lg">
      <SearchInput
        ...
        placeholder={t('filters.searchByNameOrCr')}
      />

      <Select value={filters.status ?? ''} onValueChange={(v) => onFilterChange({ status: v, page: 1 })}>

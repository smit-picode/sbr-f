...
const TABLE_COLORS: Record<string, string> = {
  SBR_ESTABLISHMENTS: 'bg-[#F4F0E8] text-[#A29374]',
...
function UserCell({ user }: { user: AuditUserRef | null | undefined }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 min-w-[120px]">
      ...
      <span className="text-xs text-[#A29374]">{user.EMAIL}</span>
    </div>
  );
}
...
function RecordIdCell({ value }: { value: number | null }) {
  ...
  return <span className="font-mono text-xs font-medium text-[#A29374]">{String(value)}</span>;
}
...
export const getAuditLogColumns = (t: TFunc): ColumnDef<AuditLog>[] => [
  ...
  },
  {
    ...
    header: t('columns.APPROVAL_REASON'),
    cell: ({ getValue }) => {
      const val = getValue<string | null>();
      return val
        ? <span className="text-sm text-slate-700">{val}</span>

...
const SRC_COLOR: Record<string, string> = {
  MOCI: '#8A1538', QFC: '#1a3a52', QFZ: '#2B7A9E', QSTP: '#B5742B', MOM_FARM: '#1F8A5B',
};

// Node colours cycled across the horizontal lifecycle timeline (oldest → newest).
const LIFECYCLE_DOT_COLORS: string[] = ['#8A1538', '#B5742B', '#1a3a52', '#1F8A5B'];
  ...
  return (
    <PageContainer>
      ...
      {lifecycleEvents.length > 0 && (
        <div className="rounded-lg bg-white shadow-card">
          <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
            <Activity className="h-4 w-4 text-[#8A1538]" />
      ...
      {legalUnits.length > 0 && (
        <div className="rounded-lg bg-white shadow-card">
          <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
            <Table className="h-4 w-4 text-[#8A1538]" />
            ...
            <button
              ...
              className="ms-auto inline-flex items-center gap-1 text-xs font-semibold text-[#8A1538] hover:underline"
            >
          ...
          <div className="grid grid-cols-1 gap-3 p-4 lg:grid-cols-2">
            {legalUnits.map((lu, i) => (
              <button
                ...
                className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left transition-colors hover:border-[#8A1538]/40 hover:bg-[#FCF4F6] ${lu.current ? 'border-slate-200' : 'border-slate-200 bg-slate-50/50'}`}
              >
                <span

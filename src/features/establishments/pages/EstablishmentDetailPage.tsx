...
const SRC_COLOR: Record<string, string> = {
  MOCI: '#A29374', QFC: '#1a3a52', QFZ: '#2B7A9E', QSTP: '#B5742B', MOM_FARM: '#1F8A5B',
};

// Node colours cycled across the horizontal lifecycle timeline (oldest → newest).
const LIFECYCLE_DOT_COLORS: string[] = ['#A29374', '#B5742B', '#1a3a52', '#1F8A5B'];
  ...
  return (
    <div className="relative min-w-0" ref={wrapRef}>
      ...
      >
        ...
        <div className={`mt-1 text-[13px] font-semibold leading-snug text-slate-800 ${mono ? 'break-all font-mono text-[12px]' : ''}`}>{value}</div>
        {!isEmpty(source) && <p className="mt-0.5 text-[9.5px] font-semibold text-slate-400">· {source}</p>}
      </button>
      {open && (
        <FieldHistoryPopover
          anchorRef={wrapRef}
  ...
  return (
    <div className="rounded-lg bg-white shadow-card">
      <div className="border-b border-slate-100 px-4 py-2.5">
        <h2 className="text-[11px] font-semibold tracking-[0.1em] text-slate-400">{title}</h2>
      </div>
  ...
  return (
    <div className="relative flex min-w-0 items-center gap-2.5" ref={wrapRef}>
      ...
      >
        ...
        <div className="truncate text-[14px] font-bold text-slate-800">{value}</div>
      </button>
      {open && (
        <FieldHistoryPopover
          anchorRef={wrapRef}
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description={
          e.NAME_ARA ? (
            ...
            >
              <span className="truncate">{e.NAME_ARA}</span>
            </HeaderNameField>
          ) : undefined
        }
        ...
        chips={
          <>
            ...
            {e.EST_STATUS && <StatusBadge status={e.EST_STATUS} className="rounded-full" />}
        ...
        actions={
          <div className="flex shrink-0 items-center gap-2">
            ...
            {canEdit && (
              <Button onClick={() => setEditOpen(true)} className="rounded-full bg-dune text-white hover:bg-dune-deep">
      ...
      {lifecycleEvents.length > 0 && (
        <div className="rounded-lg bg-white shadow-card">
          <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
            <Activity className="h-4 w-4 text-[#A29374]" />
      ...
      {legalUnits.length > 0 && (
        <div className="rounded-lg bg-white shadow-card">
          <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
            <Table className="h-4 w-4 text-[#A29374]" />
            ...
            <button
              type="button"
              onClick={() => router.push(`/legal-units?establishment=${encodeURIComponent(String(e.SBR_ID))}`)}
              className="ms-auto inline-flex items-center gap-1 text-xs font-semibold text-[#A29374] hover:underline"
            >
          ...
          <div className="grid grid-cols-1 gap-3 p-4 lg:grid-cols-2">
            {legalUnits.map((lu, i) => (
              ...
              >
                ...
                {lu.current ? (
                  <span className="shrink-0 rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9.5px] font-bold text-emerald-700">

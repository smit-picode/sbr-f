...
export function AddressDetailPage({ addressId }: { addressId: number }) {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={title}
        description={a.ZONE ? `Zone ${a.ZONE}` : undefined}
        back={{ label: t('addressDetail.allAddresses'), onClick: () => router.push('/addresses') }}
        chips={
          <>
            <span className="inline-flex items-center rounded-full bg-white/15 px-2 py-0.5 font-mono text-xs text-white">
